...
function result(sessions) {
  ...
  sessions.forEach(element => {
    if ( !result[element.session_id]) {
      result[element.session_id] = {
        session_id: element.session_id,
        time: element.time
      }
    }

    if (result[element.session_id]["classes"] === undefined) {
      result[element.session_id]["classes"] = []
    }

    result[element.session_id]["classes"].push({
      class_id: element.class.class_id,
      name: element.class.name,
    })

    if (result[element.session_id]["classes"]["students"] === undefined) {
      result[element.session_id]["classes"]["students"] = []
    }

    result[element.session_id]["classes"]["students"].push({
      student_id: element.student.student_id,
